...
if [[ -f "${ROOT}/.persius_isaac_ros_common-config" ]]; then
    . "${ROOT}/.persius_isaac_ros_common-config"
fi
...
echo "Prevent running as root." >> $LOG_FILE
if [[ $(id -u) -eq 0 ]]; then
...
# Check if all LFS files are in place - RR commenting out all this code as rev-parse always errors out
# echo "Check if all LFS files are in place" >> $LOG_FILE
...
echo "Remove any exited containers" >> $LOG_FILE
...
# Build image
echo "Build image" >> $LOG_FILE
IMAGE_KEY=ros2_humble
if [[ ! -z "${CONFIG_IMAGE_KEY}" ]]; then
    IMAGE_KEY=$CONFIG_IMAGE_KEY
    echo "Image key found, using the following CONFIG_IMAGE_KEY:$CONFIG_IMAGE_KEY" >> $LOG_FILE
fi

echo "get the base image key" >> $LOG_FILE
...
echo "building base image as image" >> $LOG_FILE
echo "BASE_IMAGE_KEY:$BASE_IMAGE_KEY"  >> $LOG_FILE
...
echo "looking for error in building base image as image" >> $LOG_FILE
if [ $? -ne 0 ]; then
...
echo "Map host's display socket to docker" >> $LOG_FILE
...
echo "Optionally load custom docker arguments from file" >> $LOG_FILE
...
xhost +  # allow containers to communicate with X server
# rob updated user from admin to root to test
# rob updated docker run to remove the "--rm" option, which was causing the container to be removed after exiting
docker run -it \
    --privileged \
    --network host \
    ${DOCKER_ARGS[@]} \
